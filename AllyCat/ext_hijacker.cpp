    ...
    if (cSubKeys) {
        ...
        for (i = 0; i < cSubKeys; i++) { 
            ...
            if (retCode == ERROR_SUCCESS) {
				std::wstring arr_w(achKey);
				std::string arr_s(arr_w.begin(), arr_w.end());
				subkeys.push_back(arr_s);
...
{
    HKEY hDstKey = NULL;
	std::wstring  app_subkey(subKey.begin(), subKey.end());
    if (RegCreateKey(dstBaseKey, app_subkey.c_str(), &hDstKey) != ERROR_SUCCESS) {
        return false;
    }
